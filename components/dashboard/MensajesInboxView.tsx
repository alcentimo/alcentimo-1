...
export interface MensajesInboxViewProps {
...
/**
 * Punto de entrada cliente de la bandeja de mensajes.
 * InboxSessionProvider envuelve toda la UI para persistir chat activo, filtros y borradores.
 */
export function MensajesInboxView(props: MensajesInboxViewProps) {
  return (
    <InboxSessionProvider initialConversations={props.initialConversations}>
      <MensajesInboxWorkspace {...props} />
    </InboxSessionProvider>
  );
}

function MensajesInboxWorkspace({
  ...
  if (initialConversations.length === 0) {
    return (
      <div className="mensajes-inbox-view" data-inbox-session="v2">
  ...
  if (facebookConversations.length === 0) {
    return (
      <div className="fb-inbox fb-inbox--session mensajes-inbox-view" data-inbox-session="v2">
        <FbInboxTopBar
          filters={listFilters}
  ...
  return (
    <div
      className="fb-inbox fb-inbox--session mensajes-inbox-view"
      data-inbox-session="v2"
    >
      <FbInboxTopBar
        filters={listFilters}
        onFiltersChange={setListFilters}
