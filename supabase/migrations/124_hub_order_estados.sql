-- Flujo de dropshipping con centro de acopio:
-- pendiente → procesando → preparacion_logistica → enviado
-- Conserva por_pagar / entregado / cancelado por compatibilidad.
...
BEGIN
  ...
  -- El stock del Hub ya se congeló al crear el pedido.
  -- Al despachar (enviado/entregado) se confirma la salida de inventario local.
  IF p_new_estado IN ('enviado', 'entregado')
     AND v_old_estado NOT IN ('enviado', 'entregado') THEN
...
COMMENT ON CONSTRAINT orders_estado_check ON public.orders IS
  'Pipeline hub: por verificar pago, pago aprobado, preparación logística, enviado.';
